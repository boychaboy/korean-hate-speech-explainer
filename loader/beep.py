...
class HateSpeechProcessor(DataProcessor):
    ...
    def __init__(self, configs, tokenizer=None):
        ...
        self.task_mode = configs.task_mode
    ...
    def _create_examples(self, data_dir, split, label=None):
        ...
        if self.task_mode == 'hate':
            ...
            for i, line in enumerate(f.readlines()[1:]):
                line = line.split('\t')
                example = InputExample(text_a=line[0].strip(), guid='%s-%s' % (split, i))
                setattr(example, 'label', labels[line[3].strip()])
                examples.append(example)

        elif self.task_mode == 'bias':
            labels = {'none': 0,
                      'gender': 1,
                      'others': 1,
                      }
            for i, line in enumerate(f.readlines()[1:]):
                line = line.split('\t')
                example = InputExample(text_a=line[0].strip(), guid='%s-%s' % (split, i))
                setattr(example, 'label', labels[line[2].strip()])
                examples.append(example)

        elif self.task_mode == 'gender-bias':
            labels = {'False': 0,
                      'True': 1,
                      }
            for i, line in enumerate(f.readlines()[1:]):
                line = line.split('\t')
                example = InputExample(text_a=line[0].strip(), guid='%s-%s' % (split, i))
                setattr(example, 'label', labels[line[1].strip()])
                examples.append(example)

        else:
            sys.exit("Unknown task_mode : {self.task_mode}")
    ...
    def get_dataloader(self, split, batch_size=1):
        ...
        dataset = HateSpeechDataset(features)
