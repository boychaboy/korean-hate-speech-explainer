...
def acc_and_f1(preds, labels, pred_probs):
    ...
    try:
        ...
        roc = 0.0
    return {"acc": acc, "f1": f1, "precision": p, "recall": r, "auc_roc": roc}
...
def main():
    ...
    parser.add_argument("--negative_weight", default=1.0, type=float)
    ...
    parser.add_argument(
        "--explain", action="store_true", help="if true, explain test set predictions"
    )
    ...
    parser.add_argument(
        "--discard_other_nw", action="store_false", dest="keep_other_nw"
    )
    ...
    # 각자 만든 프로세서 추가
    processors = {"beep": HateSpeechProcessor}

    output_modes = {"beep": "classification"}
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                if args.reg_explanations:
                    reg_loss, reg_cnt = explainer.compute_explanation_loss(
                        input_ids, input_mask, segment_ids, label_ids, do_backprop=True
                    )
...
def validate(
    ...
    for input_ids, input_mask, segment_ids, label_ids in tqdm(
        ...
        if args.reg_explanations:
            with torch.no_grad():
                reg_loss, reg_cnt = explainer.compute_explanation_loss(
                    input_ids, input_mask, segment_ids, label_ids, do_backprop=False
                )
...
def explain(args, model, processor, tokenizer, output_mode, label_list, device):
    ...
    for i, (input_ids, input_mask, segment_ids, label_ids) in tqdm(
        enumerate(eval_dataloader), desc="Evaluating"
    ):
        if i == args.stop:
            break
        if hiex_idxs and i not in hiex_idxs:
            continue
        input_ids = input_ids.to(device)
        input_mask = input_mask.to(device)
        segment_ids = segment_ids.to(device)
        ...
        if not args.hiex:
            explainer.word_level_explanation_bert(
                input_ids, input_mask, segment_ids, label_ids
            )
        else:
            explainer.hierarchical_explanation_bert(
                input_ids, input_mask, segment_ids, label_ids
            )
