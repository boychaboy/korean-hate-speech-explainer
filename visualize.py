...
def visualize_tabs(tab_file, model_name, method_name):
    ...
    :return:
    """
    # use tokenizer to join tokens into words
    ...
    for i, entry in enumerate(data):
        ...
        if score_array.shape[1] <= 400:
            ...
            if not os.path.isdir(dir):
                os.mkdir(dir)
            plt.savefig(
                "figs/{}_{}/fig_{}.png".format(model_name, method_name, i),
                bbox_inches="tight",
            )
            plt.close()
...
def main():
    ...
    parser.add_argument(
        "--input_file", default=None, type=str, required=True,
    )
    parser.add_argument("--model_name", type=str, required=True)
    parser.add_argument(
        "--hiex", action="store_true",
    )

    args = parser.parse_args()

    if args.hiex:
        visualize_tabs(args.input_file, args.model_name, "hiex")
    else:
        visualize_sequences(args.input_file, args.model_name, "seq")
    print("Done")
